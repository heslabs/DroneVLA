# Drone SoM

| Feature | NVIDIA | Qualcomm | Mediatek | Raspberry Pi |
|:-|:-|:-|:-|:-|
| SoM | [Jetson AGX Orin](https://www.nvidia.com/en-us/autonomous-machines/embedded-systems/jetson-orin/) | [Dragonwing IQ9](https://www.qualcomm.com/internet-of-things/products/iq9-series) | [Genio 1200](https://www.mediatek.com/products/iot/genio-iot/genio-1200) | [Raspberry Pi 5](https://www.raspberrypi.com/products/raspberry-pi-5/) |
| CPU | Cortex-A78AE | Cortex-A78 | Cortex-A78 | Cortex-A76 |
| Memory | Up to 64GB | Up to 32GB | Up to 32GB | Up to 16GB |
| GPU | NVDIA Ampere GPU | Adreno 663 GPU | Arm Mali-G57 MC5 | VideoCore VII |
| NPU | NVDIA Ampere GPU | In-House NPU | In-House NPU | Halio M.2 Accelerator |
| ISP | NVDIA Ampere GPU| Supported | Supported | N/A |
| H.264 | NVDIA Ampere GPU | Supported | Supported | N/A |
| OS | Ubuntu | Ubuntu | Ubuntu | Pi OS (Ubuntu) |
| ROS2 | Supported | Supported | Supported | Supported |
| AI Tools | CUDA | [AI Hub](https://aihub.qualcomm.com/) | In-House | [Halio AI SDK](https://hailo.ai/products/hailo-software/hailo-ai-software-suite/) |
| AI Model Zoo | CUDA | [AI Hub](https://aihub.qualcomm.com/models) | TBD | [Halio AI Model Zoo](https://github.com/hailo-ai/hailo_model_zoo) |


---
### Raspberry Pi AI HAT+ 2

* Introducing the Raspberry Pi AI HAT+ 2: Generative AI on Raspberry Pi 5
    * https://www.raspberrypi.com/news/introducing-the-raspberry-pi-ai-hat-plus-2-generative-ai-on-raspberry-pi-5/
* Raspberry Pi AI HAT+ 2: Run Generative AI Locally
    * https://hailo.ai/resources/industries/personal-compute/on-device-genai-on-raspberry-pi-ai-hat-2-with-hailo-10h/

---
### Radxa Dragon Q6A
* Radxa Dragon Q6A
    * https://radxa.com/products/dragon/q6a/

---
### Hiwonder MentorPi
* Hiwonder MentorPi A1 Raspberry Pi 5 Robot Car, Ackerman Chassis 2DOF Monocular Camera ROS2-HUMBLE Support SLAM and Autonomous Driving (USD 299)
    * https://www.hiwonder.com/products/mentorpi-a1-monocular-camera-version?variant=41370001244247

---
<img width="750" height="350" alt="image" src="https://github.com/user-attachments/assets/10199155-da61-4068-ab1c-97811c80f4ce" />
 
<img width="600" height="600" alt="image" src="https://github.com/user-attachments/assets/a0105edf-2cc7-4a97-93ab-9d8cfe5bd51c" />

<img width="1599" height="1065" alt="image" src="https://github.com/user-attachments/assets/de226021-d291-4841-a139-35f197a08f39" />
